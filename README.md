...
- ⌨️ **自动捕获**：剪切板变化即记录，零输入成本
- 🔍 **本地全文搜索**：SQLite FTS5，毫秒级查找历史
- ⚡ **快速粘贴面板**：`Cmd+Shift+V`（macOS）/ `Ctrl+Shift+V`（Windows）唤起最近记录
- 🛡️ **端侧敏感拦截**：密码 / Token / JWT / 银行卡号 / 身份证 / 手机号 端侧检测，绝不上云
- 📋 **App 黑白名单**：1Password / Bitwarden / 银行客户端等敏感来源默认不记录
- 🌐 **域名规则库**：搜索页自动跳过解析，内容页（抖音 / B 站 / 知乎 / 公众号 / YouTube / X / Reddit / Medium 等）自动识别
- 📦 **数据导出**：JSON / Markdown 一键导出，数据永远归你
- 🔕 **可暂停可关闭**：一键暂停记录，数据全部保留
...
4. **重要**：Teamo 需要 **辅助功能** 权限读取剪切板。系统设置 → 隐私与安全性 → 辅助功能 → 启用 Teamo
...
**Teamo** · 你的人生记录 Agent · 由 [刘智宇](https://github.com/mrliuzhiyu) 与 [Claude](https://claude.com/claude-code) 共同设计
