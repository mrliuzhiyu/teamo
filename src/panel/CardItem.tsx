...
export default function CardItem({ row, selected, query, onMouseEnter, onCopy, onForget }: Props) {
  ...
  return (
    <div
      ...
      className={`relative px-3 py-2 border-b border-stone-100 cursor-pointer transition-colors group ${
        selected ? "bg-stone-100" : "bg-white hover:bg-stone-50"
      }`}
    >
      ...
      {selected && (
        <div className="absolute right-2 top-2 flex items-center gap-1">
          ...
          <button
            ...
            className="px-2 py-0.5 text-[11px] bg-white border border-stone-200 rounded hover:bg-red-50 hover:border-red-200 hover:text-red-600 text-stone-600"
            title="忘记这条"
          >
            忘记
          </button>
